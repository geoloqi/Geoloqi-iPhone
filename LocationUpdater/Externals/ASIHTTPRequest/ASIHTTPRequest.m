...
NSString *ASIHTTPRequestVersion = @" 2010-04-06";
...
- (void)startRequest
{
	...
	NSLog(@"Request full\n%@%@", [[[NSString alloc] initWithData:(NSData *)data  encoding:NSUTF8StringEncoding] autorelease],
		  [[[NSString alloc] initWithData:[self postBody]  encoding:NSUTF8StringEncoding] autorelease]);
#endif
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	NSLog(@"Request failed: %@ with error: %@", self, theError);
#endif
	[self setComplete:YES];
